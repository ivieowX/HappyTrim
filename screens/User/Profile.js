...
const Profile = ({ route }) => {
  ...
  const profileData = [
    ...
    { key: "4", label: "อายุ:", value: user.age || "N/A" },
  ];
  const renderItem = ({ item }) => (
    <View>
      <Text style={styles.profileEmail}>{item.label} {item.value}</Text>
    </View>
  );

  // เพิ่มการนำทางไปยัง Card เมื่อคุณต้องการ



  const handleUserAboutPress = () => {
    navigation.navigate("UserScreenAbout");
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.section}>
        ...
        <TouchableOpacity style={styles.row} onPress={handleUserAboutPress}>
