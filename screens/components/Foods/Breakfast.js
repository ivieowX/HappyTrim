...
const Breakfast = () => {
    const [diet_breakfast, setdiet_breakfast] = useState([]);
    ...
    const fetchUserData = async () => {
        try {
            const response = await fetch(
                "http://mgt2.pnu.ac.th/jakpong/6460704003/diet_breakfast.php"
            );
            const data = await response.json();
            if (data.success) {
                setdiet_breakfast(data.diet_breakfast);
                setLoading(false);
            } else {
    ...
    const handleCardPress = (item) => {
        navigation.navigate('FoodCard', { item, mealType: 'อาหารเช้า' }); 
    };
    ...
    const totalCalories = diet_breakfast.reduce((sum, item) => sum + parseFloat(item.calorie || 0), 0);
    const totalItems = diet_breakfast.length;
